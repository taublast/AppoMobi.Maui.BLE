...
namespace AppoMobi.Maui.BLE
{
    public partial class Device
    {
        ...
        internal Device(Maui.BLE.Adapter adapter, BluetoothDevice nativeDevice, BluetoothGatt gatt, int rssi, byte[] advertisementData = null) : this(adapter)
        {
        ...
        private void ConnectToGattForceBleTransportAPI(bool autoconnect, CancellationToken cancellationToken)
        {
            ...
            if (Build.VERSION.SdkInt < BuildVersionCodes.Lollipop)
            {
                ...
                var connectGatt = NativeDevice.ConnectGatt(Platform.CurrentActivity, autoconnect, _gattCallback);
            ...
            else if (Build.VERSION.SdkInt < BuildVersionCodes.M)
            {
                ...
                m.Invoke(NativeDevice, Platform.CurrentActivity, false, _gattCallback, transport);
            }
            else
            {
                var connectGatt = NativeDevice.ConnectGatt(Platform.CurrentActivity, autoconnect, _gattCallback, BluetoothTransports.Le);
                _connectCancellationTokenRegistration.Dispose();
                _connectCancellationTokenRegistration = cancellationToken.Register(() => DisconnectAndClose(connectGatt));
            }
        }

        private async Task<IReadOnlyList<Maui.BLE.Service>> DiscoverServicesInternal()
        {
            return await TaskBuilder
                .FromEvent<IReadOnlyList<Maui.BLE.Service>, EventHandler<ServicesDiscoveredCallbackEventArgs>, EventHandler>(
                    execute: () =>
                    {
                    ...
                    {
                        complete(_gatt.Services.Select(service => new Maui.BLE.Service(service, _gatt, _gattCallback, this)).ToList());
                    }),
        ...
        {
            var manager = (BluetoothManager)Platform.CurrentActivity.GetSystemService(Context.BluetoothService);
        ...
        private async Task<IReadOnlyList<Maui.BLE.Service>> GetServicesNativeAsync()
        {
            ...
            {
                return new List<Maui.BLE.Service>();
            }

            // _gatt.Services is already populated if device service discovery was already done
            if (_gatt.Services.Any())
            {
                return _gatt.Services.Select(service => new Maui.BLE.Service(service, _gatt, _gattCallback, this)).ToList();
            }
        ...
        private async Task<Maui.BLE.Service> GetServiceNativeAsync(Guid id)
        {
            ...
            {
                return new Maui.BLE.Service(nativeService, _gatt, _gattCallback, this);
            }
        ...
        internal void Connect(ConnectParameters connectParameters, CancellationToken cancellationToken)
        {
            ...
            {
                var connectGatt = NativeDevice.ConnectGatt(Platform.CurrentActivity, connectParameters.AutoConnect, _gattCallback);
                _connectCancellationTokenRegistration.Dispose();
                _connectCancellationTokenRegistration = cancellationToken.Register(() => DisconnectAndClose(connectGatt));
            }
        }
