...
namespace AppoMobi.Maui.BLE
{
	public partial class Characteristic
	{
		...
		public Characteristic(CBCharacteristic nativeCharacteristic, CBPeripheral parentDevice, Maui.BLE.Service service, IBleCentralManagerDelegate bleCentralManagerDelegate) : this(service)
		{
		...
		protected Task<IReadOnlyList<Maui.BLE.Descriptor>> GetDescriptorsNativeAsync()
		{
			var exception = new Exception($"Device '{Service.Device.Id}' disconnected while fetching descriptors for characteristic with {Id}.");

			return TaskBuilder.FromEvent<IReadOnlyList<Maui.BLE.Descriptor>, EventHandler<CBCharacteristicEventArgs>, EventHandler<CBPeripheralErrorEventArgs>>(
				execute: () =>
				{
				...
				getCompleteHandler: (complete, reject) => (sender, args) =>
				{
					...
					{
						complete(args.Characteristic.Descriptors.Select(descriptor => new Maui.BLE.Descriptor(descriptor, _parentDevice, this, _bleCentralManagerDelegate)).Cast<Maui.BLE.Descriptor>().ToList());
					}
				},
